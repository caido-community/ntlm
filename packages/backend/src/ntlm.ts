...
import type { MessageType2 } from "./types.js";
...
export function createType1Message(
  workstation: string | undefined = undefined,
  ...
  // Could use os.hostname() instead, but it's not available in Caido
 ...
 */
export function decodeType2Message(authHeader: string): MessageType2 {
  ...
  const buf = Buffer.from(ntlmMatch[1], "base64");
  const obj: MessageType2 = {};

  // check signature
  if (buf.toString("ascii", 0, NTLMSIGNATURE.length) !== NTLMSIGNATURE) {
    throw new Error("Invalid message signature: " + authHeader);
  }
